...
6. API Key、密码、Token 等真实 Secret 禁止提交到仓库。优先使用环境变量，例如 `DASHSCOPE_API_KEY`。
   本项目不是生产系统，允许在本地未提交的开发数据库或本地配置中使用固定测试密码，便于启动和联调；但 API Key、Token、真实生产凭据以及其他可复用的第三方 Secret 仍然禁止写入 Git。
...
- 后端 Java 代码修改需要运行时验证时，先停止旧进程并执行 `mvn clean`，再重新编译/测试和启动，避免使用旧 `target/classes` 验证。
