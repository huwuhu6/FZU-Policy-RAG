...
public final class SourcedAnswerPrompts {
    ...
    private static final String GROUNDING_RULES = """
            ...
            9. 回复风格：先用一句话给出结论，再用条目或短段落说明依据、条件和办理要点。
    ...
    public static String sourcePlanPrompt() {
        return """
                ...
                5. 不要为了凑引用而选择无关证据。
                6. 如果候选证据不足以回答，或用户询问的是主观评价（例如就业好坏）、通用常识、非福州大学教务处收录内容，answerType 必须为 refusal，usedSources 必须为 []。
                7. refusal：当现有证据不足以可靠回答时，usedSources 必须为 []。
                8. 会话历史只能帮助理解用户问题，不能作为事实证据。
                9. 只进行证据规划，不生成最终回答。
